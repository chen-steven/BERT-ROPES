...
from model_mask_mc_all import BertForQAandMLM
from torch.utils.data import DataLoader, RandomSampler
import torch.nn.functional as F
from dataset_mask_mc_all import ROPES
import utils
import evaluate
...
def test(args, model, dev_dataset, tokenizer, contrast=False):
    with open(dev_dataset.evaluation_file, 'r') as f:
    ...
    for step, batch in enumerate(dev_iterator):
        ...
        batch_size = batch['input_ids0'].size(0)
        pred = torch.argmax(outputs[1], dim=1)
        for i in range(batch_size):
            answers[qids[i]] = evaluation[qids[i]][pred[i]]
...
def main():
    ...
    parser.add_argument("--output_dir", default="train/ropes10_0.5_mc_all", type=str)
    ...
    parser.add_argument('--find_mask', default="label2", type=str)
    ...
    train_dataset = ROPES(tokenizer, 'ropes_no_coref_train_combined_examples.json',
                          'train_processed_candidate_answers.json', find_mask=args.find_mask)
    dev_dataset = ROPES(tokenizer, 'ropes_no_coref_dev_combined_examples.json',
                        'dev_processed_candidate_answers.json', eval=True, find_mask=args.find_mask)
    contrast_dataset = ROPES(tokenizer, 'ropes_no_coref_contrast_examples.json',
                             'contrast_processed_candidate_answers.json', eval=True, find_mask=args.find_mask)
